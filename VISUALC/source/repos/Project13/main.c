...
int main(int argc, char* argv[]) {
	...
	for (rcounter = 0; rcounter < 12; rcounter++)
	{
		...
		scanf_s("%i", &ptr2Calendar[rcounter]->id);
		while ((c = getchar()) != '\n' && c != EOF) {} 	// clears input buffer	

		printf("\nPlease enter the name of the boy or girls: ");
		//gets(Calendar[rcount].name);
		gets_s(ptr2Calendar[rcounter]->name, sizeof(ptr2Calendar[rcounter]->name));
		//while ((c = getchar()) != '\n' && c != EOF) { } 
		...
		scanf_s("%i", &ptr2Calendar[rcounter]->day);
		while ((c = getchar()) != '\n' && c != EOF) {} 	// clears input buffer
		...
		scanf_s("%i", &ptr2Calendar[rcounter]->month);
		while ((c = getchar()) != '\n' && c != EOF) {} 	// clears input buffer
	}
