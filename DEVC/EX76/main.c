...
int main(void)
{
	...
	*/
    ...
    char full_id[113];
  	...
  	strcpy(full_id, client2.name);
  	strcat(full_id, " ");       // Concatenating a space
  	//strcat(full_id, client1.address);
  	strcat(full_id, client2.address);
  	strcat(full_id, " ");       // Concatenating a space
  	//strcat(full_id, client1.phone);
  	strcat(full_id, client2.phone);
	printf("\n\nConcatenation:\n%s", full_id );
	...
	{
        if(isalpha(full_id[i])) letterCount++;
	}
    ...
    for(int i = 0; i < sizeof(full_id); i++) 
	{
        if(isdigit(full_id[i])) digitCount++;
	}
